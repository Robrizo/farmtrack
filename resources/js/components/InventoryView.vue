      ...
      <main>
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
            ...
            <table :class="{'hidden': isVisible}" class="w-full text-center shadow-lg table-auto">
              ...
              <caption class="bg-lightGray p-3">
                <div class="flex text-center">
                  <h2 class="uppercase font-bold text-lg text-left text-primaryColor">manage farm inputs</h2>
                  <form action="" class="ml-auto">
                    <div class="flex items-center justify-center">
                    <input v-model="searchKeyword" @input="searchInput" class="px-2 py-1 rounded-l-lg outline-none" type="search" name="" placeholder="Search...">
                    <button type="submit">
              ...
              <thead class="text-sm uppercase bg-gray-200">
                <tr class="border border-gray-400">
                  ...
                  <th class="p-4 border border-gray-400">input name</th>
                  <th class="p-4 border border-gray-400">quantity</th>
                  <th class="p-4 border border-gray-400">unit price (Mk)</th>
                  <th class="p-4 border border-gray-400">total (MK)</th>
                  <th class="p-4 border border-gray-400">purchase date</th>
              ...
              <tbody class="capitalize text-lg bg-white">
                <tr v-for="(item, index) in inputDetails.data" :key="item.id" class="border border-gray-400">
                  <td class="p-2 border border-gray-400">{{ index + 1 }}</td>
                  <td class="p-2 border border-gray-400">{{ item.input_name }}</td>
                  <td class="p-2 border border-gray-400">{{ item.quantity }}</td>
                  <td class="p-2 border border-gray-400">{{item.unit_price}}</td>
                  <td class="p-2 border border-gray-400">{{item.total}}</td>
                  <td class="p-2 border border-gray-400">{{ item.purchase_date }}</td>
                  <td>
              ...
              <tfoot class="bg-gray-200">
                <tr >
                <td colspan="7" class="p-3 border border-gray-400">
                  <div class="">
                    <TailwindPagination
                      :data="inputDetails"
                      @pagination-change-page="fetchFarmInventory" 
                    />
                  </div>
                </td>
              </tr>
          ...
          <div v-if="isVisible" class="center-form">
            <form @submit.prevent.self="addNewInput" class="max-w-md mx-auto mb-8 bg-white p-8 rounded-lg shadow-lg">
              <div class="text-2xl font-bold mb-6 text-center text-primaryColor capitalize">
                <h1>add farm input</h1>
              </div>
              <div>
                <label for="input_name" class="block text-lg font-semibold mb-2 capitalize">input name</label>
                <input v-model="newInput.input_name" id="input_name" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="text" name="" placeholder="enter input" required>
              </div>
              <div>
                <label for="quantity" class="block text-lg font-semibold mb-2 capitalize">quantity</label>
                <input v-model.number="newInput.quantity" @input="calculateTotal" id="quantity" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="number" name="" placeholder="enter quantity" required>
              </div>
              <div>
                <label for="unit_price" class="block text-lg font-semibold mb-2 capitalize">unit price</label>
                <input v-model.number="newInput.unit_price" @input="calculateTotal" id="unit_price" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="number" name="" placeholder="enter unit price" required>
              </div>
              <div>
                <label for="total" class="block text-lg font-semibold mb-2 capitalize">total</label>
                <input v-model.number="newInput.total" id="total" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="number" name="" placeholder="total" disabled>
              </div>
              <div>
                <label for="purchase_date" class="block text-lg font-semibold mb-2 capitalize">purchase date</label>
                <input v-model="newInput.purchase_date" id="purchase_date" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="date" name="" required>
              </div>
        ...
        <div v-if="isFormVisible" class="fixed inset-0 bg-gray-800 bg-opacity-50 w-full flex items-center justify-center">
          <div class="bg-white p-8 max-w-md rounded-md">
            ...
            <form @submit.prevent="saveChanges">
              <!-- form fields  -->
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">input name</label>
            <input v-model="newInput.input_name" type="text" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">quantity</label>
            <input v-model.number="newInput.quantity" @input="calculateTotal" type="number" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">unit price</label>
            <input v-model.number="newInput.unit_price" @input="calculateTotal" type="number" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">total</label>
            <input v-model.number="newInput.total" type="number" class="mt-1 p-2 w-full border outline-primaryColor rounded-md" disabled>
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">purchase date</label>
            <input v-model="newInput.purchase_date" type="date" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
...
export default {
  ...
  methods: {
    ...
    calculateTotal () {
      this.newInput.total = this.newInput.quantity * this.newInput.unit_price
    },
    ...
    async addNewInput () {
      try {
        const response = await axios.post('/api/farminventory', this.newInput)
        this.inputDetails.data.push(response.data.farms)
        this.closeForm()
        this.newInput = { input_name: '', quantity: '', unit_price: '', total: 0, purchase_date: '' }
    ...
    // get inventory data in the db method
    async fetchFarmInventory (page = 1) {
      try {
        const response = await axios.get(`/api/farminventory?page=${page}`);
        this.inputDetails = response.data.farms
      } catch (error) {
    ...
    // save edited record to db method
    async saveChanges () {
      try {
        const response = await axios.put(`/api/farminventory/${this.newInput.id}`, this.newInput)
        this.inputDetails = response.data.farms
        this.closeEditForm()
        this.fetchFarmInventory()
    ...
    async deleteInput (id) {
      try {
        ...
        if (result.isConfirmed) {
          await axios.delete(`/api/farminventory/${id}`)
          this.inputDetails = this.inputDetails.data.filter(item => item.id !== id)
          this.fetchFarmInventory()
