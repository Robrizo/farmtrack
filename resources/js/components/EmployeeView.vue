    ...
    <main>
      <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          ...
          <table :class="{'hidden': isVisible}" class="w-full text-center shadow-lg table-auto">
            ...
            <caption class="bg-lightGray p-3">
              <div class="flex text-center">
                <h2 class="uppercase font-bold text-lg text-left text-primaryColor">manage employees</h2>
                <form @submit.prevent="searchEmployee" action="" class="ml-auto">
                  <div class="flex items-center justify-center">
                    <input v-model="searchKeyword" @input="searchEmployee" class="px-2 py-1 rounded-l-lg outline-none" type="search" name="" placeholder="Search...">
                    <button type="submit">
            ...
            <thead class="text-sm uppercase bg-gray-200">
              <tr class="border border-gray-400">
                ...
                <th class="p-4 border border-gray-400">firstname</th>
                <th class="p-4 border border-gray-400">lastname</th>
                <th class="p-4 border border-gray-400">position</th>
                <th class="p-4 border border-gray-400">phone number</th>
                <th class="p-4 border border-gray-400">address</th>
                <th class="p-4 border border-gray-400">salary (mk)</th>
            ...
            <tbody class="capitalize text-lg bg-white">
              <tr v-for="(item, index) in employeeDetails.data" :key="item.id" class="border border-gray-400">
                <td class="p-2 border border-gray-400">{{ index + 1 }}</td>
                <td class="p-2 border border-gray-400">{{ item.firstname }}</td>
                <td class="p-2 border border-gray-400">{{ item.lastname }}</td>
                <td class="p-2 border border-gray-400">{{item.position}}</td>
                <td class="p-2 border border-gray-400">{{item.phone_number}}</td>
                <td class="p-2 border border-gray-400">{{ item.address }}</td>
                <td class="p-2 border border-gray-400">{{ item.salary }}</td>
                <td>
                  <button @click="editFormVisible (item)">
                    <span class="mr-6"><i class="bx bx-edit"></i></span>
                  </button>
                  <button @click="deleteEmployee (item.id)">
            ...
            <tfoot class="bg-gray-200">
              <tr >
                <td colspan="8" class="p-3 border border-gray-400">
                  <div class="">
                    <TailwindPagination
        ...
        <div v-if="isVisible" class="center-form">
          <form @submit.prevent.self="addNewEmployee" class="max-w-md mx-auto mb-8 bg-white p-8 rounded-lg shadow-lg">
            <div class="text-2xl font-bold mb-6 text-center text-primaryColor capitalize">
              <h1>add employee</h1>
            </div>
            <div>
              <label for="firstname" class="block text-lg font-semibold mb-2 capitalize">firstname</label>
              <input v-model="newEmployee.firstname" id="firstname" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="text" name="" placeholder="enter firstname" required>
            </div>
            <div>
              <label for="lastname" class="block text-lg font-semibold mb-2 capitalize">lastname</label>
              <input v-model="newEmployee.lastname" id="lastname" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="text" name="" placeholder="enter lastname" required>
            </div>
            <div>
              <label for="position" class="block text-lg font-semibold mb-2 capitalize">position</label>
              <input v-model="newEmployee.position" id="position" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="text" name="" placeholder="enter position" required>
            </div>
            <div>
              <label for="phone_number" class="block text-lg font-semibold mb-2 capitalize">phone number</label>
              <input v-model="newEmployee.phone_number" id="phone_number" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="text" name="" placeholder="phone number" required>
            </div>
            <div>
              <label for="address" class="block text-lg font-semibold mb-2 capitalize">address</label>
              <input v-model="newEmployee.address" id="address" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="text" name="" placeholder="enter address" required>
            </div>
            <div>
              <label for="address" class="block text-lg font-semibold mb-2 capitalize">salary</label>
              <input v-model.number="newEmployee.salary" id="address" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="number" name="" placeholder="enter salary" required>
            </div>
        ...
        <div v-if="isFormVisible" class="fixed inset-0 bg-gray-800 bg-opacity-50 w-full flex items-center justify-center">
          <div class="bg-white p-8 max-w-md rounded-md">
            ...
            <form @submit.prevent="saveChanges">
              <!-- form fields  -->
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">firstname</label>
            <input v-model="newEmployee.firstname" type="text" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">lastname</label>
            <input v-model="newEmployee.lastname" type="text" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">position</label>
            <input v-model="newEmployee.position" type="text" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">phone number</label>
            <input v-model="newEmployee.phone_number" type="text" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">address</label>
            <input v-model="newEmployee.address" type="text" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-black capitalize">salary</label>
            <input v-model.number="newEmployee.salary" type="number" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
...
export default {
  ...
  methods: {
    ...
    async addNewEmployee () {
      try {
        const response = await axios.post('/api/employees', this.newEmployee)
        this.employeeDetails.data.push(response.data.employees)
        this.closeForm()
        this.newEmployee = { firstname: '', lastname: '', position: '', phone_number: '', address: '', salary: '' }
    ...
    async saveChanges () {
      try {
        const response = await axios.put(`/api/employees/${this.newEmployee.id}`, this.newEmployee)
        this.employeeDetails = response.data.employees
        this.closeEditForm()
        this.fetchEmployees()
    ...
    async deleteEmployee (id) {
      try {
        ...
        if (result.isConfirmed) {
          ...
          this.employeeDetails = this.employeeDetails.data.filter(item => item.id !== id)
          this.fetchEmployees()
