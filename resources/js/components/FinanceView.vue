    ...
    <main>
      <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          ...
          <table :class="{'hidden': isVisible}" class="w-full text-center shadow-lg table-auto">
            ...
            <caption class="bg-lightGray p-3">
              <div class="flex text-center">
                <h2 class="uppercase font-bold text-lg text-left text-primaryColor">manage finances</h2>
                <form @submit.prevent="searchFinance" action="" class="ml-auto">
                  <div class="flex items-center justify-center">
                    <input v-model="searchKeyword" @input="searchFinance" class="px-2 py-1 rounded-l-lg outline-none" type="search" name="" placeholder="Search...">
                    <button type="submit">
            ...
            <thead class="text-sm uppercase bg-gray-200">
              <tr class="border border-gray-400">
                ...
                <th class="p-4 border border-gray-400">transaction type</th>
                <th class="p-4 border border-gray-400">amount (mk)</th>
                <th class="p-4 border border-gray-400">transaction date</th>
                <th class="p-4 border border-gray-400">description</th>
            ...
            <tbody class="capitalize text-lg bg-white">
              <tr v-for="(item, index) in financeDetails.data" :key="item.id" class="border border-gray-400">
                <td class="p-2 border border-gray-400">{{ index + 1 }}</td>
                <td class="p-2 border border-gray-400">{{ item.trans_type }}</td>
                <td class="p-2 border border-gray-400">{{ item.amount }}</td>
                <td class="p-2 border border-gray-400">{{item.trans_date}}</td>
                <td class="p-2 border border-gray-400 overflow-auto">{{item.description}}</td>
                <td>
                  <button @click="editFormVisible (item)">
                    <span class="mr-6"><i class="bx bx-edit"></i></span>
                  </button>
                  <button @click="deleteFinance (item.id)">
              ...
              <tr >
                <td colspan="6" class="p-3 border border-gray-400">
                  <div class="">
                    <TailwindPagination
                      :data="financeDetails"
                      @pagination-change-page="fetchFinanceData"
                    >
                  </TailwindPagination>
                  </div>
                </td>
        ...
        <div v-if="isVisible" class="">
          <form @submit.prevent.self="addNewFinance" class="max-w-md mx-auto mb-8 bg-white p-8 rounded-lg shadow-lg">
            <div class="text-2xl font-bold mb-6 text-center text-primaryColor capitalize">
              <h1>add finance record</h1>
            </div>
            <div>
              <label for="trans_type" class="block text-lg font-semibold mb-2 capitalize">transaction type</label>
              <input v-model="newFinance.trans_type" id="trans_type" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="text" name="" placeholder="enter trans type" required>
            </div>
            <div>
              <label for="amount" class="block text-lg font-semibold mb-2 capitalize">amount</label>
              <input v-model.number="newFinance.amount" id="amount" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="number" name="" placeholder="enter amount" required>
            </div>
            <div>
              <label for="trans_date" class="block text-lg font-semibold mb-2 capitalize">transaction date</label>
              <input v-model="newFinance.trans_date" id="trans_date" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="date" name="" placeholder="" required>
            </div>
            <div>
              <label for="desc" class="block text-lg font-semibold mb-2 capitalize">description</label>
              <input v-model="newFinance.description" id="desc" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="text" name="" placeholder="enter description" required>
            </div>
        ...
        <div v-if="isFormVisible" class="fixed inset-0 bg-gray-800 bg-opacity-50 w-full flex items-center justify-center">
          <div class="bg-white p-8 max-w-md rounded-md">
            ...
            <form @submit.prevent="saveChanges">
              <!-- form fields  -->
          <div class="mb-4">
            <label class="block text-sm font-bold text-bold capitalize">Transaction Type</label>
            <input v-model="newFinance.trans_type" type="text" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-bold capitalize">amount</label>
            <input v-model.number="newFinance.amount" type="number" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-bold capitalize">transaction date</label>
            <input v-model="newFinance.trans_date" type="date" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-bold capitalize">description</label>
            <input v-model="newFinance.description" type="text" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
...
export default {
  ...
  methods: {
    ...
    async addNewFinance () {
      try {
        const response = await axios.post('/api/finances', this.newFinance)
        this.financeDetails.data.push(response.data.finances)
        this.closeForm()
        this.newFinance = { trans_type: '', amount: '', trans_date: '', description: '' }
    ...
    // get finance data in the db method
    async fetchFinanceData (page = 1) {
      try {
        const response = await axios.get(`/api/finances?page=${page}`);
        this.financeDetails = response.data.finances
      } catch (error) {
    ...
    async saveChanges () {
      try {
        const response = await axios.put(`/api/finances/${this.newFinance.id}`, this.newFinance)
        this.financeDetails = response.data.finances
        this.closeEditForm()
        this.fetchFinanceData()
    ...
    async deleteFinance (id) {
      try {
        ...
        if (result.isConfirmed) {
          ...
          this.financeDetails = this.financeDetails.data.filter(item => item.id !== id)
          this.fetchFinanceData()
