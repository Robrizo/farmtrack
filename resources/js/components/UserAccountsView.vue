        ...
        <div class="relative">
          <a @click.prevent="dropdownOpen" href="#" class="flex items-center gap-4">
            <span class="text-left">
              ...
              <span class="block text-base font-bold text-primaryColor capitalize">{{ user.name }}</span>
            </span>
      ...
      <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <div class="overflow-auto">
          <div class="text-right">
            <button @click="makeFormVisible" :class="{'hidden': isVisible}" class="bg-primaryColor font-bold uppercase rounded-lg p-2 px-4 mb-4 text-white">add user</button>
          </div>
          <!-- table start -->
          <table :class="{'hidden': isVisible}" class="w-full text-center shadow-lg table-auto">
            ...
            <caption class="bg-lightGray p-3">
              <div class="flex text-center">
                <h2 class="uppercase font-bold text-lg text-left text-primaryColor">manage users</h2>
                <form  @submit.prevent="searchUser" action="" class="ml-auto">
                  <div class="flex items-center justify-center">
                    <input v-model="searchKeyword" @input="searchUser" class="px-2 py-1 rounded-l-lg outline-none" type="search" name="" placeholder="Search...">
                    <button type="submit">
            ...
            <thead class="text-sm uppercase bg-gray-200">
              <tr class="border border-gray-400">
                ...
                <th class="p-4 border border-gray-400">username</th>
            ...
            <!-- table header end -->
            <!-- table body start -->
            <tbody class="text-lg bg-white">
              <tr v-for="(item, index) in userDetails.data" :key="item.id" class="border border-gray-400">
                <td class="p-2 border border-gray-400">{{ index + 1 }}</td>
                <td class="p-2 border border-gray-400">{{ item.name }}</td>
                <td class="p-2 border border-gray-400">{{ item.email }}</td>
                <td>
            ...
            <tfoot class="bg-gray-200">
              <tr >
                <td colspan="4" class="p-3 border border-gray-400">
                  <div class="">
                    <TailwindPagination
                      :data="userDetails"
        ...
        <div v-if="isVisible" class="">
          <form @submit.prevent.self="addNewUser" class="max-w-md mx-auto mb-8 bg-white p-8 rounded-lg shadow-lg">
            <div class="text-2xl font-bold mb-6 text-center text-primaryColor capitalize">
              <h1>add user</h1>
            </div>
            <div>
              <label for="trans_type" class="block text-lg font-semibold mb-2 capitalize">username</label>
              <input v-model="newUser.name" id="trans_type" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="text" name="" placeholder="enter username" required>
            </div>
            <div>
              <label for="amount" class="block text-lg font-semibold mb-2 capitalize">email</label>
              <input v-model="newUser.email" id="amount" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="email" name="" placeholder="enter email" required>
            </div>
            <div>
              <label for="trans_date" class="block text-lg font-semibold mb-2 capitalize">password</label>
              <input v-model="newUser.password" id="trans_date" class="w-full p-2 border border-gray-300 rounded outline-primaryColor placeholder:capitalize mb-4" type="password" name="" placeholder="enter password" required>
            </div>
        ...
        <div v-if="isFormVisible" class="fixed inset-0 bg-gray-800 bg-opacity-50 w-full flex items-center justify-center">
          <div class="bg-white p-8 max-w-md rounded-md">
            <h2 class="text-xl text-primaryColor font-bold mb-4">Edit user</h2>
            <form @submit.prevent="saveChanges">
              <!-- form fields  -->
          <div class="mb-4">
            <label class="block text-sm font-bold text-bold capitalize">username</label>
            <input v-model="newUser.name" type="text" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-bold capitalize">email</label>
            <input v-model="newUser.email" type="email" class="mt-1 p-2 w-full border outline-primaryColor rounded-md">
          </div>
  ...
  methods: {
    ...
    async addNewUser () {
      try {
        ...
        this.userDetails.data.push(response.data.user)
        this.closeForm()
        this.newUser = {}
    ...
    // get users data in the db method
    ...
    async deleteUser (id) {
      try {
        ...
        if (result.isConfirmed) {
          ...
          this.userDetails = this.userDetails.data.filter(item => item.id !== id)
          this.fetchUsers()
